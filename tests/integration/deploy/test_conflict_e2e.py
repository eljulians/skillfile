...
from skillfile.core.conflict import has_conflict, read_conflict
from skillfile.core.models import LockEntry
from skillfile.deploy.install import cmd_install
from skillfile.exceptions import InstallError
from skillfile.patch import has_patch, read_patch
from skillfile.patch.resolve import cmd_resolve
...
def test_install_update_clean_merge_both_changes_present(tmp_path, capsys):
    ...
    # Pin the edit
    from skillfile.patch.pin import cmd_pin
    ...
    with mock_patch("skillfile.sources.sync.sync_entry", side_effect=sync_fn):
...
def test_resolve_recovers_user_edits_and_clears_conflict(tmp_path, capsys):
    ...
    with (
        ...
        mock_patch(
            "skillfile.patch.resolve._open_in_editor",
